...
interface Stats {
    ...
    pages_viewed: number;
    active_days: number;
    popular_pages: Array<{ page_path: string; views: number }>;
...
const processUserAgents = (agents: UserAgent[], totalViews: number): UserAgent[] => {
    return agents.map(agent => ({
        ...agent,
        percentage: Number(((agent.count / totalViews) * 100).toFixed(1))
    }));
};
...
const CountryTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
        ...
        const CountryFlag = (Flags as any)[data.country];
        return (
            <Card sx={{ p: 1, backgroundColor: 'rgba(43, 43, 43, 0.9)' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    {CountryFlag && <CountryFlag style={{ width: 20 }} />}
                    <Typography>{getName(data.country) || data.country}</Typography>
...
const processCountryData = (countries: CountryData[]): CountryData[] => {
    ...
    return countries.map(country => ({
        ...country,
        percentage: Number(((country.visits / totalVisits) * 100).toFixed(1))
    }));
};

const aggregateCountryData = (countries: any[]) => {
    return Object.values(
        countries.reduce((acc: any, curr) => {
            if (!acc[curr.country]) {
                acc[curr.country] = { ...curr };
            } else {
                acc[curr.country].visits += curr.visits;
            }
            return acc;
        }, {})
    );
...
export const Stats = () => {
    ...
    const processedAgents = processUserAgents(stats.user_agents, stats.total_views);
                                    ...
                                    <ResponsiveContainer width="100%" height={300}>
                                        <PieChart>
                                            <Pie
                                                ...
                                                label={({ country, x, y }) => {
                                                    const CountryFlag = (Flags as any)[country];
                                                    return (
                                            ...
                                            >
                                                {processedCountries.map((entry: CountryData) => (
                                                    <Cell
                                                        ...
                                                        fill={getRandomColor(entry.country)}
                                                    />
                                                ))}
                                            </Pie>
                                            ...
                                            >
                                                <Box
                                                    sx={{
                                                        ...
                                                        borderRadius: '50%',
                                                        flexShrink: 0,
                                                        bgcolor: getRandomColor(country.country)
                                                    }}
                                                />
                                                {(() => {
                                                    const FlagComponent = (Flags as any)[country.country];
                                            ...
                                            <Box sx={{ position: 'relative', width: 24, height: 24 }}>
                                                <svg width="24" height="24" viewBox="0 0 24 24">
                                                    ...
                                                    <circle
                                                        ...
                                                        strokeDasharray={`${(agent.percentage / 100) * (2 * Math.PI * 10)} ${2 * Math.PI * 10}`}
                                                    />
                                                </svg>
                                            </Box>
                                            {/* OS Icon */}
